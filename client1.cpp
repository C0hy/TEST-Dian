...
int main(int argc, char* argv[])
{
	...
	while(cho[0]!='T')
	{
	    ...
	    if(cho[0]=='O')
	    {
			...
			CSeq++;
	        char  sendData[255];
		    Os(sendData,url,CSeq,username,password);
			...
			if(te->content[0]=='T'||te->content[0]=='W')
			{
				break;
			}
			//由服务端返回的内容获得客户端的PORT
	    ...
	    else if(cho[0]=='S')
	    {
			if(OPTIONS==0)
			{
				printf("You have not done method-OPTIONS!\n");
				continue;
			}
			SETUP=1;
			//按协议发送信息
	    ...
	    else if(cho[0]=='P')
	    {
			if(SETUP==0)
			{
				printf("You have not done method-SETUP!\n");
				continue;
			}
			//按协议发送信息
			CSeq++;
	        ...
	        char  sendData[255];
			session_id++;
			itoa(session_id,Csession_id,10);
